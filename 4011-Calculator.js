...
TEUI.Calculator = (function() {
    ...
    const CONSTANTS = {
        ...
        FAHRENHEIT_TO_CELSIUS: function(f) { return (f - 32) * 5/9; },
    ...
    function recalculateClimateZone() {
        ...
        if (window.TEUI.LocationManager) {
            const location = window.TEUI.LocationManager.getLocationData(city, province);
            if (location) {
        ...
        if (climateZone) {
            stateManager.setValue('climate-zone', climateZone, stateManager.VALUE_STATES.CALCULATED);
        }
    }
    ...
    function recalculateGroundFacingHDD() {
        ...
        // Update the field
        stateManager.setValue('d_22', groundFacingHDD.toString(), stateManager.VALUE_STATES.CALCULATED);
    }
    ...
    function recalculateGroundFacingCDD() {
        ...
        // Update the field
        stateManager.setValue('cf_e_22', groundFacingCDD.toString(), stateManager.VALUE_STATES.CALCULATED);
    }
    ...
    function recalculateTemperatures() {
        ...
        if (!isNaN(heatingC)) {
            ...
            stateManager.setValue('cf_e_23a', heatingF.toString(), stateManager.VALUE_STATES.CALCULATED);
        }
        ...
        if (!isNaN(coolingC)) {
            ...
            stateManager.setValue('cf_e_24a', coolingF.toString(), stateManager.VALUE_STATES.CALCULATED);
        }
    }
    ...
    function recalculateTEUIPercent() {
        ...
        // Update the field
        stateManager.setValue('cf_j_10', `${percent}%`, stateManager.VALUE_STATES.CALCULATED);
    }
    ...
    function recalculateAnnualCarbonPercent() {
        ...
        // Update the field
        stateManager.setValue('j_8', `${percent}%`, stateManager.VALUE_STATES.CALCULATED);
    }
            ...
            calculationOrder.forEach(fieldId => {
                ...
                if (field.calculate) {
                    try {
                        ...
                        stateManager.setValue(fieldId, value, stateManager.VALUE_STATES.CALCULATED);
    ...
    function calculateAll() {
        ...
        // Calculate sections in dependency order - may need to be re-ordered as some key values deopend on TEDI/TEUI sections
        calculateSection(SECTIONS.BUILDING_INFO);
        calculateSection(SECTIONS.CLIMATE);
        calculateSection(SECTIONS.KEY_VALUES);
        calculateSection(SECTIONS.ENERGY_CARBON);
        calculateSection(SECTIONS.EMISSIONS);
        calculateSection(SECTIONS.RENEWABLE);
        calculateSection(SECTIONS.WATER);
        calculateSection(SECTIONS.AIR_QUALITY);
        calculateSection(SECTIONS.INTERNAL_GAINS);
        calculateSection(SECTIONS.RADIANT_GAINS);
        calculateSection(SECTIONS.TRANSMISSION_LOSSES);
        calculateSection(SECTIONS.VOLUME_METRICS);
        calculateSection(SECTIONS.MECHANICAL);
        calculateSection(SECTIONS.TEDI);
        calculateSection(SECTIONS.TEUI);
    ...
    function initializeWeatherHandlers() {
        ...
        // Add a short delay to ensure the DOM elements are available
        setTimeout(function() {
        ...
        }, 500); // 0.5 second delay to ensure rendering is complete
    }
    ...
    function updateWeatherData(province, city) {
        ...
        const occupancyType = document.querySelector('[data-dropdown-id="dd_d_12"]')?.value || 'commercial';
        
        // Update HDD Field - both temporary and rendered cells
        const hddValue = isFuture ? cityData.data.HDD18_2021_2050 : cityData.data.HDD18;
        ...
        const cddValue = isFuture ? cityData.data.CDD24_2021_2050 : cityData.data.CDD24;
        ...
        if (occupancyType === 'residential') {
            designTemp = isFuture ? cityData.data.January_2_5_2021_2050 : cityData.data.January_2_5;
        } else {
            designTemp = isFuture ? cityData.data.January_1_2021_2050 : cityData.data.January_1;
        }
        ...
        const hottestTemp = isFuture ? 
                            (cityData.data.Extreme_Hot_Tdb_2021_2050 || cityData.data.July_2_5_Tdb_2021_2050) : 
                            (cityData.data.Extreme_Hot_Tdb_1991_2020 || cityData.data.July_2_5_Tdb || '34'); // Fallback value
    ...
    function updateFieldValue(fieldId, value) {
        // Update in DOM
        const fieldElements = document.querySelectorAll(`[data-field-id="${fieldId}"]`);
    ...
    function showFullWeatherData(province, city) {
        try {
            ...
            if (!modalEl) {
                console.error('Weather data modal element not found with ID: weatherDataModal');
    ...
    function calculateGFHDD() {
        ...
        const heatingSetpoint = heatingSetpointInput ? parseFloat(heatingSetpointInput.value) || 18 : 18;
    ...
    function calculateGFCDD() {
        ...
        const coolingSetpoint = coolingSetpointInput ? parseFloat(coolingSetpointInput.value) || 24 : 24;
...
function initializeWeatherDataHandlers() {
    ...
    const feedbackArea = document.getElementById('feedback-area');
    if (feedbackArea) {
        feedbackArea.textContent = 'Weather handlers initialized. Select a province and city to load weather data.';
        feedbackArea.style.color = '#0dcaf0';
    }
    ...
    if (provinceDropdown) {
        provinceDropdown.addEventListener('change', function(e) {
            ...
            if (province && TEUI.ExcelLocationHandler.getLocationData()) {
                const locationData = TEUI.ExcelLocationHandler.getLocationData();
                if (feedbackArea) {
                    feedbackArea.textContent = `Loading cities for ${locationData[province].name}...`;
                }
                ...
                if (feedbackArea) {
                    feedbackArea.textContent = `Loaded ${cities.length} cities for ${locationData[province].name}`;
                }
            }
        });
    }
