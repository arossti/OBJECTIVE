...
window.TEUI.CoolingCalculations = (function() {
    ...
    function calculateAtmosphericValues() {
        ...
        const pSatAvg = 610.94 * Math.exp(17.625 * state.nightTimeTemp / (state.nightTimeTemp + 243.04));
        
        // Calculate partial pressure of water vapor
        const partialPressure = pSatAvg * state.coolingSeasonMeanRH;
        
        // Calculate indoor saturation vapor pressure
        const pSatIndoor = 610.94 * Math.exp(17.625 * state.coolingSetTemp / (state.coolingSetTemp + 243.04));
    ...
    function calculateHumidityRatios() {
        ...
        const humidityRatioIndoor = 0.62198 * state.partialPressureIndoor / 
        ...
        const humidityRatioAvg = 0.62198 * state.partialPressure / 
    ...
    function calculateFreeCoolingLimit() {
        ...
        const sensibleCooling = totalMass * state.specificHeatCapacity * temperatureDifference / 1000;
        const latentAdjustment = totalMass * state.latentHeatVaporization * state.humidityRatioDifference / 1000;
        ...
        const dailyFreeCooling = (sensibleCooling - latentAdjustment > 0) ? 
    ...
    function updateStateManager() {
        ...
        sm.setValue('cooling_freeCoolingLimit', state.freeCoolingLimit.toString(), 'calculated');
        sm.setValue('cooling_daysActiveCooling', state.daysActiveCooling.toString(), 'calculated');
