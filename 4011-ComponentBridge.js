...
TEUI.ComponentBridge = (function() {
    ...
    function createDropdownHTML(fieldId, options, selectedValue) {
        ...
        optionsHTML += formattedOptions.map(option => {
        ...
        switch (field.type) {
            case 'dropdown':
                ...
                try {
                    ...
                    console.error(`Error getting options for dropdown ${dropdownId}:`, error);
                }
                ...
                innerHTML = createDropdownHTML(fieldId, dropdownOptions, field.defaultValue);
                break;
            ...
            case 'derived':
                ...
                element.classList.add(field.type === 'calculated' ? 'calculated-value' : 'derived-value');
    ...
    function initSection(sectionId) {
        const fields = TEUI.getFieldsBySection ? TEUI.getFieldsBySection(sectionId) : {};
    ...
    function initializeDropdown(dropdownElement, dropdownId) {
        ...
        if (window.TEUI && window.TEUI.StateManager) {
            ...
            if (savedValue) {
                ...
                const defaultOption = Array.from(dropdownElement.options).find(opt => opt.value === 'Present');
                if (defaultOption) {
