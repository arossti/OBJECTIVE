...
*   **S16 Sankey Tooltip/Label Data on First Paint**: In Section 16, when the Sankey diagram is first activated, emissions-related tooltips may not show values until the "Show/Hide Emissions" toggle is cycled or the diagram is refreshed. Additionally, node labels for emissions sinks (e.g., "E1 Scope 1 Emissions") might incorrectly display "(0.00 kg CO2e/yr)" on the first paint even when `showEmissions` is initially false, instead of just the node name. The data preparation in `buildSankeyDataFromStateManager` needs to ensure that `updateEmissionsFlows` is called appropriately to populate link/node data for tooltips from the outset, without breaking the emissions toggle functionality or incorrectly formatting node labels when emissions are hidden.
