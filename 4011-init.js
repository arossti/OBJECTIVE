...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeTabs() {
        ...
        sections.forEach(section => {
            ...
            const tooltip = tabTooltips[section.id] || sectionHeader.textContent.trim();
            ...
            tab.addEventListener('click', function(e) {
                ...
                if (appWrapper && document.body.classList.contains('horizontal-layout')) {
                    window.scrollTo({
    ...
    function activateTab(sectionId) {
        ...
        const selectedTab = document.querySelector(`.tab[data-section-id="${sectionId}"]`);
        if (selectedTab) {
    ...
    document.querySelectorAll('.section-header').forEach(header => {
        header.addEventListener('click', function(event) {
            ...
            if (event.target.closest('.btn') || event.target.closest('.layout-toggle-btn')) {
                return;
            }
    ...
    if (layoutToggleButton) {
        layoutToggleButton.addEventListener('click', function(e) {
            ...
            localStorage.setItem('layoutMode', isVerticalLayout ? 'vertical' : 'horizontal');
        });
    }
    ...
    function updateSectionHeights() {
        ...
        if (keyValuesSection) {
            ...
            document.documentElement.style.setProperty('--key-values-height', `${keyValuesHeight}px`);
        }
    }
    ...
    function positionTabContainer() {
        if (keyValuesSection && tabContainer) {
            ...
            document.documentElement.style.setProperty('--key-values-height', `${keyValueRect.height}px`);
        }
    }
    
    // Make key values close button same size as layout toggle
    const headerButtons = document.querySelectorAll('.section-header .btn, .layout-toggle-btn');
                ...
                fileInput.addEventListener('change', function(e) {
                    ...
                    if (file) {
                        ...
                        TEUI.ExcelLocationHandler.loadExcelFile(file)
                            .then(() => {
                                feedbackArea.textContent = 'Excel data loaded successfully! Click Apply to use the data.';
        ...
        if (applyExcelBtn) {
            applyExcelBtn.addEventListener('click', function() {
                if (TEUI.ExcelLocationHandler.getLocationData()) {
                    ...
                    feedbackArea.textContent = 'Location data applied. Choose a province and city to continue.';
                    applyExcelBtn.classList.add('d-none');
                } else {
                    feedbackArea.textContent = 'No location data available. Please select an Excel file first.';
                }
            });
        }
