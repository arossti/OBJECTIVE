...
const EnvelopeCalculations = {
  ...
  componentHeatLoss(geometry, thermal, climate) {
    ...
    return {
      ...
      reference: calcLoss(
        geometry.area,
        thermal.reference.rsi,
        climate.reference.hdd,
      ),
    };
  },
  ...
  wallHeatLoss(inputs) {
    ...
    components.forEach((comp) => {
      ...
      const thermal = {
        ...
        reference: {
          rsi: parseFloat(inputs.reference[`f_${comp.row}`]) || 2.97,
        },
      };
  ...
  windowHeatLoss(inputs) {
    ...
    components.forEach((comp) => {
      ...
      const thermal = {
        ...
        reference: {
          rsi: parseFloat(inputs.reference[`f_${comp.row}`]) || 0.35,
        },
      };
  ...
  totalEnvelopeLoss(inputs) {
    ...
    const tbPenalty = {
      target: parseFloat(inputs.target.d_97) || 0.1, // User-defined
      reference: parseFloat(inputs.reference.d_97) || 0.1, // Same as user
    };
...
function demonstrateEnvelopeCalculation() {
  ...
  console.log(
    `  Window Loss: ${result.target.windows.total.toFixed(0)} kWh/yr`,
  );
  console.log(
    `  Thermal Bridge: ${result.target.thermalBridgeLoss.toFixed(0)} kWh/yr`,
  );
  ...
  console.log(
    `  Window Loss: ${result.reference.windows.total.toFixed(0)} kWh/yr`,
  );
  console.log(
    `  Thermal Bridge: ${result.reference.thermalBridgeLoss.toFixed(0)} kWh/yr`,
  );
