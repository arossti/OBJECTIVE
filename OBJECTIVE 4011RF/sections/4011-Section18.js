...
window.TEUI.SectionModules.sect18 = (function () {
   ...
   */
  function generateAndDisplayQCReport() {
    if (!qcEnabled || !window.TEUI?.QCMonitor) {
      updateQCOutput('QC Monitor not available. Enable QC monitoring first.');
      return;
    }
    
    console.log('[S18] Generating QC report...');
    
    // Generate comprehensive report
    const report = window.TEUI.QCMonitor.generateQCReport();
    lastReport = report;
    ...
    console.log('[S18] QC Report generated - see Notes section for formatted output');
  }
  ...
  function formatReportAsHTML(report) {
    ...
    let html = `
      <div style="padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; padding-bottom: 15px; border-bottom: 3px solid #007bff;">
          <div>
            <h5 style="margin: 0; color: #007bff; font-weight: bold;">🔍 TEUI QC MONITORING REPORT</h5>
    ...
    // Add detailed violations (no scrolling, just expand)
    if (report.violations && report.violations.length > 0) {
      html += `
        <div style="margin-bottom: 25px;">
          <h6 style="color: #495057; margin-bottom: 15px;">🔍 Detailed Violations</h6>
      `;
      
      report.violations.slice(0, 15).forEach((violation, index) => { // Show more violations since we have space
        const emoji = violation.severity === 'error' ? '❌' : 
                     violation.severity === 'warning' ? '⚠️' : 'ℹ️';
        const bgColor = violation.severity === 'error' ? '#fff5f5' : 
                       violation.severity === 'warning' ? '#fffbf0' : '#f8f9fa';
        const borderColor = violation.severity === 'error' ? '#dc3545' : 
                           violation.severity === 'warning' ? '#ffc107' : '#dee2e6';
        
        html += `
          <div style="background: ${bgColor}; padding: 12px; margin: 8px 0; border-radius: 6px; border-left: 4px solid ${borderColor};">
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
              <div style="flex: 1;">
                <strong style="color: ${borderColor};">${emoji} ${violation.type}</strong>
                <div style="font-size: 12px; color: #666; margin: 4px 0;">Field: <code>${violation.field}</code></div>
                <div style="font-size: 11px; color: #333;">${violation.message}</div>
              </div>
              ${violation.expected ? `<div style="font-size: 10px; color: #666; text-align: right; margin-left: 15px;">Expected: ${violation.expected}<br>Actual: ${violation.actual}</div>` : ''}
            </div>
          </div>
        `;
      });
      ...
      if (report.violations.length > 15) {
        html += `<div style="text-align: center; color: #666; font-style: italic; margin-top: 15px; padding: 10px; background: #f8f9fa; border-radius: 6px;">... and ${report.violations.length - 15} more violations (check console for full list)</div>`;
      }
...
### Top Violations:
${report.violations.slice(0, 20).map(v => `- **${v.type}**: \`${v.field}\` - ${v.message}`).join('\n')}
