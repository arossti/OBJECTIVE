...
window.TEUI.SectionModules.sect09 = (function () {
  ...
  //==========================================================================
  // HELPER FUNCTIONS (CORRECTED FOR 100% STATE ISOLATION - NO FALLBACKS)
  //==========================================================================
  function getNumericValue(fieldId, defaultValue = 0) {
    const prefix = ModeManager.currentMode === "target" ? "target_" : "ref_";
    const rawValue = window.TEUI?.StateManager?.getValue(`${prefix}${fieldId}`);
    // NO FALLBACK to global state. If a value is missing, it's a state initialization issue.
  ...
  function setFieldValue(fieldId, value, fieldType = "calculated") {
    ...
    // DOM updates are handled by listeners on the StateManager for 'target_*' fields.
    // We no longer need to update the global state or DOM directly from here.
  }

  function setCalculatedValue(fieldId, rawValue, formatType = "number") {
    // This function now correctly uses the isolated setFieldValue
  ...
  function calculatePlugLoads() {
    ...
    const validOccupiedHours = isNaN(occupiedHours) ? 0 : occupiedHours;
    const energy = (plugLoadDensity * conditionedArea * validOccupiedHours) / 1000;
    ...
    if (plugLoadDensity <= referencePlugLoad) {
      setFieldValue("n_65", "✓");
      updateElementClass("n_65", "checkmark");
    } else {
      setFieldValue("n_65", "✗");
      updateElementClass("n_65", "warning");
    }
  ...
  function calculateLightingLoads() {
    ...
    const occupiedHours = getNumericValue("i_63");
    const validOccupiedHours = isNaN(occupiedHours) ? 0 : occupiedHours;
    const energy = (lightingDensity * conditionedArea * validOccupiedHours) / 1000;
    ...
    if (percentOfReference <= 133) {
      setFieldValue("n_66", "✓");
      updateElementClass("n_66", "checkmark");
    } else {
      setFieldValue("n_66", "✗");
      updateElementClass("n_66", "warning");
    }
  ...
  function calculateEquipmentLoads() {
    ...
    const occupiedHours = getNumericValue("i_63");
    const validOccupiedHours = isNaN(occupiedHours) ? 0 : occupiedHours;
    const energy = (densityValue * conditionedArea * validOccupiedHours) / 1000;
    ...
    setFieldValue("n_67", "✓");
    updateElementClass("n_67", "checkmark");
   ...
   */
  function updateElementClass(fieldId, className) {
    ...
    editableFields.forEach((field) => {
      ...
      field.addEventListener("blur", function () {
        ...
        if (window.TEUI?.StateManager) {
          ...
          // REMOVED: Redundant global state update.
        }
  ...
  function addStateManagerListeners() {
    ...
    const dependencies = [
      { source: "target_h_15", handler: calculateTargetModel, description: "Update target when conditioned area changes" },
      { source: "ref_h_15", handler: calculateReferenceModel, description: "Update reference when conditioned area changes" },
      { source: "target_d_54", handler: calculateTargetModel, description: "Update target when DHW system losses change" },
      { source: "ref_d_54", handler: calculateReferenceModel, description: "Update reference when DHW system losses change" },
      { source: "target_d_12", handler: calculateTargetModel, description: "Update target when building type changes" },
      { source: "ref_d_12", handler: calculateReferenceModel, description: "Update reference when building type changes" },
      { source: "target_d_13", handler: calculateTargetModel, description: "Update target when reference standard changes" },
      { source: "ref_d_13", handler: calculateReferenceModel, description: "Update reference when reference standard changes" },
      { source: "target_m_19", handler: calculateTargetModel, description: "Update target when cooling days change" },
      { source: "ref_m_19", handler: calculateReferenceModel, description: "Update reference when cooling days change" },
    ];
