...
window.TEUI.SectionModules.sect01 = (function () {
  ...
  //==========================================================================
  // PART 4: DUAL-ENGINE HELPER FUNCTIONS (REFACTORED FOR STRICT STATE ISOLATION)
  //==========================================================================

  /**
   * Helper function to safely get numeric values from the TARGET state.
   * CORRECTED: Reads ONLY from target_ prefixed StateManager values. NO FALLBACKS.
   */
  function getTargetNumericValue(fieldId, defaultValue = 0) {
    const targetValue = window.TEUI?.StateManager?.getValue?.(`target_${fieldId}`);
    if (targetValue !== undefined && targetValue !== null && targetValue !== "") {
      return window.TEUI?.parseNumeric?.(targetValue, defaultValue) ?? defaultValue;
    }
    return defaultValue;
  }
   ...
   * CORRECTED: Reads ONLY from ref_ prefixed StateManager values. NO FALLBACKS.
   */
  ...
  /**
   * Helper to get a string value from the TARGET state.
   * CORRECTED: Reads ONLY from target_ prefixed StateManager values. NO FALLBACKS.
   */
  function getTargetValue(fieldId, defaultValue = '') {
    const targetValue = window.TEUI?.StateManager?.getValue?.(`target_${fieldId}`);
  ...
  function calculateReferenceModel() {
    ...
    try {
      // CRITICAL: Get Reference values using the new strict helper
      const refTEUIFromS15 = getRefNumericValue("h_136", 341.2);
      const refK32FromS04 = getRefNumericValue("k_32", 14740.8);
      
      const referenceTEUI = refTEUIFromS15;

      // Calculate Reference Annual Carbon (e_8)
      const refTargetEmissions = refK32FromS04;

      const refArea = getRefNumericValue("h_15", 1427.2);

      let referenceAnnualCarbon = 0;
      if (refArea > 0) {
        referenceAnnualCarbon = refTargetEmissions / refArea;
      }

      // Calculate Reference Lifetime Carbon
      const refEmbodiedCarbon = getRefNumericValue("i_39", 0);
      const refServiceLife = getRefNumericValue("h_13", 60);
      let referenceLifetimeCarbon = 0;

      if (refServiceLife > 0) {
        referenceLifetimeCarbon = (refEmbodiedCarbon / refServiceLife) + referenceAnnualCarbon;
      }

      // Set state with ref_ prefix and update display
      if (window.TEUI?.StateManager) {
        window.TEUI.StateManager.setValue("ref_e_10", referenceTEUI.toString(), "calculated");
        window.TEUI.StateManager.setValue("ref_e_8", referenceAnnualCarbon.toString(), "calculated");
        window.TEUI.StateManager.setValue("ref_e_6", referenceLifetimeCarbon.toString(), "calculated");
      }
      
      updateDisplayValue("e_10", window.TEUI?.formatNumber?.(referenceTEUI, "number-1dp"));
      updateDisplayValue("e_8", window.TEUI?.formatNumber?.(referenceAnnualCarbon, "number-1dp"));
      updateDisplayValue("e_6", window.TEUI?.formatNumber?.(referenceLifetimeCarbon, "number-1dp"));
  ...
  /**
   * TARGET MODEL ENGINE: Calculate all Column H & K values using Target state exclusively
   */
  function calculateTargetModel() {
    ...
    try {
      // All inputs from Application state, using new strict helper
      const appTargetEnergy = getTargetNumericValue("j_32", 0);
      const appActualEnergy = getTargetNumericValue("f_32", 0);
      const appTargetEmissions = getTargetNumericValue("k_32", 0);
      const appActualEmissions = getTargetNumericValue("g_32", 0);
      const appArea = getTargetNumericValue("h_15", 1);
      const appServiceLife = getTargetNumericValue("h_13", 50);
      const appEmbodiedCarbon = getTargetNumericValue("i_41", 345.82);
      const useType = getTargetValue("d_14", "Targeted Use");

      // Calculate Target TEUI (h_10)
      let targetTEUI = 0;
      if (appArea > 0) {
        targetTEUI = appTargetEnergy / appArea;
      }

      // Calculate Actual TEUI (k_10)
      let actualTEUI = 0;
      if (useType === "Utility Bills" && appArea > 0) {
        actualTEUI = appActualEnergy / appArea;
      }

      // Calculate Target Annual Carbon (h_8)
      let targetAnnualCarbon = 0;
      if (appArea > 0) {
        targetAnnualCarbon = appTargetEmissions / appArea;
      }

      // Calculate Actual Annual Carbon (k_8)
      let actualAnnualCarbon = 0;
      if (useType === "Utility Bills" && appArea > 0) {
        actualAnnualCarbon = appActualEmissions / appArea;
      }

      // Calculate Target Lifetime Carbon (h_6)
      let targetLifetimeCarbon = 0;
      if (appServiceLife > 0) {
        targetLifetimeCarbon = (appEmbodiedCarbon / appServiceLife) + targetAnnualCarbon;
      }

      // Calculate Actual Lifetime Carbon (k_6)
      let actualLifetimeCarbon = 0;
      if (useType === "Utility Bills" && appServiceLife > 0) {
        actualLifetimeCarbon = (appEmbodiedCarbon / appServiceLife) + actualAnnualCarbon;
      }

      // Set state with target_ prefix and update display
      if (window.TEUI?.StateManager) {
        window.TEUI.StateManager.setValue("target_h_10", targetTEUI.toString(), "calculated");
        window.TEUI.StateManager.setValue("target_h_8", targetAnnualCarbon.toString(), "calculated");
        window.TEUI.StateManager.setValue("target_h_6", targetLifetimeCarbon.toString(), "calculated");
        
        if (useType === "Utility Bills") {
          window.TEUI.StateManager.setValue("target_k_10", actualTEUI.toString(), "calculated");
          window.TEUI.StateManager.setValue("target_k_8", actualAnnualCarbon.toString(), "calculated");
          window.TEUI.StateManager.setValue("target_k_6", actualLifetimeCarbon.toString(), "calculated");
        }
      }

      updateDisplayValue("h_10", window.TEUI?.formatNumber?.(targetTEUI, "number-1dp"));
      updateDisplayValue("h_8", window.TEUI?.formatNumber?.(targetAnnualCarbon, "number-1dp"));
      updateDisplayValue("h_6", window.TEUI?.formatNumber?.(targetLifetimeCarbon, "number-1dp"));

      if (useType === "Utility Bills") {
        updateDisplayValue("k_10", window.TEUI?.formatNumber?.(actualTEUI, "number-1dp"));
        updateDisplayValue("k_8", window.TEUI?.formatNumber?.(actualAnnualCarbon, "number-1dp"));
        updateDisplayValue("k_6", window.TEUI?.formatNumber?.(actualLifetimeCarbon, "number-1dp"));
      } else {
  ...
  function calculatePercentagesAndExplanations() {
    ...
    // Get calculated values from both engines using new strict helpers
    const referenceAnnualCarbon = getRefNumericValue("e_8", 17.4);
    const referenceTEUI = getRefNumericValue("e_10", 341.2);

    const targetAnnualCarbon = getTargetNumericValue("h_8", 4.7);
    const targetTEUI = getTargetNumericValue("h_10", 93.0);
    const actualAnnualCarbon = getTargetNumericValue("k_8", 4.8);
    const actualTEUI = getTargetNumericValue("k_10", 93.1);
    ...
    // Update display directly, no need for separate state for percentage strings
    updateDisplayValue("j_8", newJ8);
    updateDisplayValue("j_10", newJ10);
  ...
  function calculateTargetTier() {
    ...
    const targetTEUI_h10 = getTargetNumericValue("h_10", 93.0);
  ...
  function updateTEUIDisplay() {
    ...
    if (isUtilityMode) {
      ...
      const energyValue_f32 = getTargetNumericValue("f_32", 0);
  ...
  function getGaugeValues(gaugeId) {
    ...
    const referenceValue = getRefNumericValue(refFieldPrefixed, defaultRef);
    const appValueForGauge = getTargetNumericValue(
  ...
  function checkTargetExceedsReference() {
    const targetValue = getTargetNumericValue("h_10", 93.0);
  ...
  function initializeEventHandlers() {
    ...
    const inputFieldsToWatch = [
      "target_i_41", // Embodied carbon (user input)
      "target_h_13", // Service life (user input)
      "target_h_15", // Conditioned area (user input)
      "target_d_51", // Energy source (user dropdown)
      "target_d_14", // Use type (user dropdown)
      "target_d_13", // Reference standard (user dropdown)
      "ref_h_13",    // Ref Service life
      "ref_h_15",    // Ref Conditioned area
      "ref_i_39",    // Ref Embodied carbon
    ];
    ...
    const calculatedFieldsToWatch = [
      "target_j_32", // Target energy total (from Section 04)
      "target_k_32", // Target emissions total (from Section 04)
      "target_f_32", // Actual energy total (from Section 04)
      "target_g_32", // Actual emissions total (from Section 04)
