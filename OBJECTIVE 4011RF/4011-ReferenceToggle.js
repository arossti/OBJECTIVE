...
TEUI.ReferenceToggle = (function () {
  ...
  function getAllDualStateSections() {
    ...
    const sectionIds = ['sect02', 'sect03', 'sect04', 'sect08', 'sect10', 'sect11', 'sect12', 'sect13', 'sect14', 'sect15'];
    ...
    sectionIds.forEach(sectionId => {
      if (window.TEUI?.SectionModules?.[sectionId]) {
        ...
        if (sectionModule.ModeManager || window.TEUI?.[sectionId]?.ModeManager) {
          dualStateSections.push({
            id: sectionId,
            module: sectionModule,
            modeManager: sectionModule.ModeManager || window.TEUI?.[sectionId]?.ModeManager
          });
        }
      }
    });
    
    console.log(`[ReferenceToggle] Found ${dualStateSections.length} dual-state sections:`, dualStateSections.map(s => s.id));
    ...
    dualStateSections.forEach(section => {
      try {
        if (section.modeManager && typeof section.modeManager.switchMode === 'function') {
          ...
          switchedCount++;
          console.log(`[ReferenceToggle] Switched ${section.id} to ${mode.toUpperCase()} mode`);
        } else {
          console.warn(`[ReferenceToggle] ${section.id} has no switchMode method`);
        }
      } catch (error) {
        console.error(`[ReferenceToggle] Error switching ${section.id} to ${mode}:`, error);
      }
    });
    
    console.log(`[ReferenceToggle] Successfully switched ${switchedCount}/${dualStateSections.length} sections to ${mode.toUpperCase()} mode`);
  ...
  function updateAllCalculatedDisplays() {
    ...
    dualStateSections.forEach(section => {
      try {
        if (section.modeManager && typeof section.modeManager.updateCalculatedDisplayValues === 'function') {
          section.modeManager.updateCalculatedDisplayValues();
        } else if (section.modeManager && typeof section.modeManager.refreshUI === 'function') {
          section.modeManager.refreshUI();
        }
      } catch (error) {
        console.error(`[ReferenceToggle] Error updating display for ${section.id}:`, error);
      }
    });
  }
  ...
  function initialize() {
    ...
    const standardSelector = document.getElementById(STANDARD_SELECTOR_ID) ||
                            document.querySelector(`[data-field-id='${STANDARD_SELECTOR_ID}']`);
    if (standardSelector) {
      const actualSelect = standardSelector.tagName === "SELECT" ? 
                          standardSelector : 
  ...
  function toggleReferenceDisplay() {
    ...
    console.log(`[ReferenceToggle] Switching ALL sections to ${targetMode.toUpperCase()} display mode`);
  ...
  function handleStandardChange(event) {
    ...
    console.log(`[ReferenceToggle] Reference standard changed to: ${newStandardKey}`);
    
    if (window.TEUI?.StateManager) {
      // Update the global standard
      window.TEUI.StateManager.setValue(STANDARD_SELECTOR_ID, newStandardKey, "user-modified");
      ...
      dualStateSections.forEach(section => {
        try {
          ...
          if (section.module.ReferenceState?.onReferenceStandardChange) {
            section.module.ReferenceState.onReferenceStandardChange(newStandardKey);
            console.log(`[ReferenceToggle] Updated ${section.id} for new reference standard`);
          }
        } catch (error) {
          console.error(`[ReferenceToggle] Error updating ${section.id} for standard change:`, error);
        }
      });
  ...
  function toggleReferenceInputsView() {
    ...
    console.log(`[ReferenceToggle] Reference inputs view: ${isViewingReferenceInputs ? 'ON' : 'OFF'}`);
