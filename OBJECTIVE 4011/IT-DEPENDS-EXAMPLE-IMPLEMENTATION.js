...
function setupTEUICalculations() {
  ...
  stateManager.registerCalculation(
    "k_10",
    function () {
      const actualEnergy = window.TEUI.parseNumeric(
        stateManager.getValue("f_32"),
        132938,
      );
      const area = window.TEUI.parseNumeric(
        stateManager.getValue("h_15"),
        1427.2,
      );
  ...
  stateManager.registerCalculation(
    "h_10",
    function () {
      const targetEnergy = window.TEUI.parseNumeric(
        stateManager.getValue("j_32"),
        132765.65,
      );
      const area = window.TEUI.parseNumeric(
        stateManager.getValue("h_15"),
  ...
  stateManager.registerCalculation(
    "j_10",
    function () {
      const actualTEUI = window.TEUI.parseNumeric(
        stateManager.getValue("k_10"),
        0,
      );
      const targetTEUI = window.TEUI.parseNumeric(
        stateManager.getValue("h_10"),
        0,
      );

      if (targetTEUI <= 0) {
        console.warn(
          "[TEUI Calc] Invalid target TEUI for j_10 percentage calculation",
        );
        return "0%";
      }
...
function setupGroundFacingCalculations() {
  ...
  stateManager.registerCalculation(
    ...
    function () {
      ...
      console.log(
        `[GF Calc] Calculated d_22 (Ground Facing HDD): ${groundFacingHDD}`,
      );
  ...
  stateManager.registerCalculation(
    "e_23a",
    function () {
      const celsiusValue = window.TEUI.parseNumeric(
        stateManager.getValue("d_23a"),
        21,
      );
...
function setupSmartListeners() {
  ...
  stateManager.addSmartListener("f_32", (newValue, oldValue, fieldId) => {
    console.log(
      `[Smart Listener] Actual energy changed: ${oldValue} → ${newValue}`,
    );
  ...
  stateManager.addSmartListener("j_32", (newValue, oldValue, fieldId) => {
    console.log(
      `[Smart Listener] Target energy changed: ${oldValue} → ${newValue}`,
    );
  });

  stateManager.addSmartListener("h_15", (newValue, oldValue, fieldId) => {
    console.log(
      `[Smart Listener] Building area changed: ${oldValue} → ${newValue}`,
    );
  });
  ...
  stateManager.addSmartListener("h_23", (newValue, oldValue, fieldId) => {
    console.log(
      `[Smart Listener] Heating setpoint changed: ${oldValue} → ${newValue}`,
    );

    // Additional validation or processing if needed
    if (newValue < 18 || newValue > 24) {
      console.warn(
        "[Smart Listener] Heating setpoint outside typical range (18-24°C)",
      );
    }
  });
}
...
function demonstrateBatchCalculation() {
  ...
  const calculatedFields =
    stateManager.calculateBatchDependencies(changedFields);

  console.log(
    `[Batch Demo] Import complete. Recalculated ${calculatedFields.length} dependent fields:`,
    calculatedFields,
  );
}
...
function demonstrateManualDependencyCalculation() {
  ...
  console.log(
    `[Manual Demo] Calculated ${calculatedFields.length} fields:`,
    calculatedFields,
  );
}
...
function setupCalculationMonitoring() {
  ...
  window.monitorStateManager = function () {
    ...
    console.log(
      "[Monitor] Registered calculations:",
      Array.from(stateManager.fieldCalculations?.keys?.() || []),
    );

    console.log(
      "[Monitor] Current dirty fields:",
      stateManager.getDirtyFields(),
    );
    console.log(
      "[Monitor] Total dependencies registered:",
      debugInfo.dependencyCount,
    );
  };
}
...
function initializeCalculationOrchestration() {
  ...
  window.demonstrateManualDependencyCalculation =
...
document.addEventListener("DOMContentLoaded", function () {
  setTimeout(() => {
    if (window.TEUI && window.TEUI.StateManager) {
      ...
      console.warn(
        "[IT-DEPENDS] StateManager not available for auto-initialization",
      );
    }
  }, 100);
});
...
function testS13_d115_ITDepends() {
  ...
  if (!hasRegistration) {
    ...
    console.log(
      "Available registered calculations:",
      sm.getRegisteredCalculations
        ? sm.getRegisteredCalculations()
        : "getRegisteredCalculations not found",
    );
    return;
  }
  ...
  console.log(
    `Expected: ${expected.toFixed(2)}, Got: ${result}, Correct: ${isCorrect ? "✅" : "❌"}`,
  );
...
function testS10_ITDepends_Comprehensive() {
  ...
  const registeredCalculations = criticalCalculations.filter(
    (id) => sm.hasCalculation && sm.hasCalculation(id),
  );
  console.log(
    `✅ ${registeredCalculations.length}/${criticalCalculations.length} critical calculations registered`,
  );
  ...
  console.log(
    `Gain factors correct: ${northCorrect && southCorrect && westCorrect ? "✅" : "❌"}`,
  );
  ...
  console.log(
    `Application North heating (SHGC=0.3): ${appNorthHeating.toFixed(2)}`,
  );
  console.log(
    `Reference North heating (SHGC=0.5): ${refNorthHeating.toFixed(2)}`,
  );
  console.log(
    `Application South heating (SHGC=0.6): ${appSouthHeating.toFixed(2)}`,
  );
  console.log(
    `Reference South heating (SHGC=0.5): ${refSouthHeating.toFixed(2)}`,
  );

  // Verify SHGC normalization is working
  // Application should be different from Reference due to different SHGC values
  const shgcNormalizationWorking =
    Math.abs(appNorthHeating - refNorthHeating) > 0.1 &&
    Math.abs(appSouthHeating - refSouthHeating) > 0.1;
  console.log(
    `SHGC state handling working: ${shgcNormalizationWorking ? "✅" : "❌"}`,
  );
  ...
  console.log(
    `Total gains (solar + internal): ${totalGains.toFixed(2)} kWh/yr`,
  );
  ...
  console.log(
    `NRC 40%: Factor=${(utilizationFactor40 * 100).toFixed(1)}%, Usable=${usableGains40.toFixed(2)} kWh/yr`,
  );
  ...
  console.log(
    `NRC 60%: Factor=${(utilizationFactor60 * 100).toFixed(1)}%, Usable=${usableGains60.toFixed(2)} kWh/yr`,
  );
  ...
  console.log(
    `PH Method: Factor=${(utilizationFactorPH * 100).toFixed(1)}%, Usable=${usableGainsPH.toFixed(2)} kWh/yr`,
  );

  // Verify D80 toggle is working (different methods give different results)
  const d80Working =
    Math.abs(utilizationFactor40 - utilizationFactor60) > 0.1 &&
  ...
  console.log(
    `PHPP Reference utilization: ${(phppUtilizationFactor * 100).toFixed(1)}%`,
  );
  console.log(
    `PHPP Reference usable gains: ${phppUsableGains.toFixed(2)} kWh/yr`,
  );
  console.log(`Unusable gains: ${unusableGains.toFixed(2)} kWh/yr`);

  // Verify calculations add up
  const calculationCheck =
    Math.abs(usableGainsPH + unusableGains - totalGains) < 0.1;
  console.log(`Gain calculations balanced: ${calculationCheck ? "✅" : "❌"}`);

  // 6. Test dependency chain
  console.log("\n--- Testing Dependency Chain ---");
  console.log(
    "🔗 Testing climate zone change triggers gain factor recalculation...",
  );
  ...
  console.log(
    `South gain factor changed: ${oldSouthGainFactor} → ${newSouthGainFactor}`,
  );

  // For climate zone 7, South should be 24.76 (northern values)
  const climateChainWorking =
    Math.abs(parseFloat(newSouthGainFactor) - 24.76) < 0.01;
  console.log(
    `Climate dependency chain working: ${climateChainWorking ? "✅" : "❌"}`,
  );
  ...
  const shgcChainWorking =
    Math.abs(parseFloat(newNorthHeating) - parseFloat(oldNorthHeating)) > 0.1;
  console.log(
    `SHGC dependency chain working: ${shgcChainWorking ? "✅" : "❌"}`,
  );
  ...
  if (allTestsPassed) {
    ...
    console.log(
      "❌ Some S10 IT-DEPENDS tests failed - check individual results above",
    );
  }
...
function testS10_D80_Toggle() {
  ...
  const d80Registered = d80Calculations.every(
    (id) => sm.hasCalculation && sm.hasCalculation(id),
  );
  ...
  methods.forEach((method) => {
    ...
    console.log(
      `${method}: ${(factor * 100).toFixed(1)}% → ${usable.toFixed(0)} kWh/yr usable`,
    );
  });
  ...
  console.log(
    `D80 toggle working: ${toggleWorking ? "✅" : "❌"} (${uniqueFactors.size} unique factors)`,
  );
...
function testS10_SHGC_States() {
  ...
  const refRegistered = refCalculations.every(
    (id) => sm.hasCalculation && sm.hasCalculation(id),
  );
  ...
  const shgcStateWorking =
    refGains > appGains && Math.abs(refGains - appGains) > 10;

  console.log(`SHGC state handling working: ${shgcStateWorking ? "✅" : "❌"}`);
  console.log(
    `Reference gains ${((refGains / appGains - 1) * 100).toFixed(1)}% higher than Application`,
  );
