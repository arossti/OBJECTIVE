...
TEUI.StateManager = (function () {
   ...
   */
  function shouldAllowCrossSectionCalculation(
    triggeringSection,
    targetSection,
  ) {
    ...
    if (globalCalculationDepth.current >= MAX_GLOBAL_DEPTH) {
      console.warn(
        `[StateManager] 🚨 Cross-section recursion protection: depth ${globalCalculationDepth.current}, blocking ${triggeringSection} → ${targetSection}`,
      );
      return false;
    }
    ...
    if (activeSections.has(targetSection)) {
      console.warn(
        `[StateManager] 🚨 Cross-section recursion protection: ${targetSection} already active, blocking feedback loop`,
      );
      return false;
    }
  ...
  function loadReferenceData(standardKey) {
    ...
    Object.keys(referenceModeDefaults).forEach((fieldId) => {
      ...
      if (
        activeReferenceDataSet.hasOwnProperty(fieldId) ||
    ...
    if (window.TEUI && window.TEUI.ReferenceValues) {
      ...
      if (exactMatch) {
        ...
        console.log(
          ...
          exactMatch.hasOwnProperty("d_53"),
        );
  ...
  function setReferenceValue(fieldId, value, source) {
    ...
    if (
      source === "user-modified-reference" ||
      independentReferenceState.hasOwnProperty(fieldId)
    ) {
      independentReferenceState[fieldId] = value;
    }
  ...
  function emitFieldChanged(modelType, fieldId, newValue) {
    document.dispatchEvent(
      new CustomEvent("FieldValueChanged", {
  ...
  function registerCalculation(fieldId, calculationFn, description = "") {
    ...
    console.log(
      `[StateManager] Registered calculation for ${fieldId}: ${description}`,
    );
  }
  ...
  function triggerFieldCalculation(changedFieldId) {
    ...
    if (typeof changedFieldId === "number") {
      console.warn(
        `[StateManager] triggerFieldCalculation received numeric fieldId: ${changedFieldId}. Skipping calculation.`,
      );
      return;
    }
    ...
    if (!changedFieldId || typeof changedFieldId !== "string") {
      console.warn(
        `[StateManager] triggerFieldCalculation requires valid string fieldId, got: ${changedFieldId}`,
      );
      return;
    }
    ...
    if (calculationInProgress) {
      console.warn(
        `[StateManager] Recursion detected: already calculating for ${changedFieldId}`,
      );
      return;
    }
    ...
    if (calculationDepth >= MAX_CALCULATION_DEPTH) {
      console.warn(
        `[StateManager] Maximum calculation depth (${MAX_CALCULATION_DEPTH}) reached for ${changedFieldId}`,
      );
      return;
    }
    ...
    const currentSection = changedFieldId.includes("_")
      ? `S${changedFieldId.split("_")[1].substring(0, 2)}`
      : "Unknown";

    if (globalCalculationDepth.current > 1) {
      console.log(
        `[StateManager] 🌐 Cross-section calculation: ${currentSection} (global depth: ${globalCalculationDepth.current})`,
      );
    }
    ...
    try {
      ...
      console.log(
        `[StateManager] Triggering calculations for changed field: ${changedFieldId} (depth: ${calculationDepth})`,
      );
      ...
      if (dependentFields.length === 0) {
        console.log(
          `[StateManager] No dependent calculations found for: ${changedFieldId}`,
        );
        return;
      }

      console.log(
        `[StateManager] Found ${dependentFields.length} dependent fields: [${dependentFields.join(", ")}]`,
      );

      // Execute calculations for dependent fields
      for (const fieldId of dependentFields) {
        const targetSection = fieldId.includes("_")
          ? `S${fieldId.split("_")[1].substring(0, 2)}`
          : "Unknown";

        // Check cross-section permission before calculating (TODO: REMOVE AFTER MIGRATION)
        if (currentSection !== targetSection) {
          if (
            !shouldAllowCrossSectionCalculation(currentSection, targetSection)
          ) {
            console.log(
              `[StateManager] ⏸️ Skipping cross-section calculation: ${currentSection} → ${targetSection} (${fieldId})`,
            );
            continue;
          }
        }
        ...
        if (fieldCalculations.has(fieldId)) {
          try {
            ...
            console.log(
              `[StateManager] Calculation complete for ${fieldId}: ${result}`,
            );
          } catch (calcError) {
            console.error(
              `[StateManager] Error calculating ${fieldId}:`,
              calcError,
            );
          }
        } else {
          ...
          if (!warnedMissingCalculations.has(warningKey)) {
            console.warn(
              `[StateManager] No calculation function registered for dependent field: ${fieldId} (migration warning - shown once)`,
            );
      ...
      console.error(
        `[StateManager] Error in triggerFieldCalculation for ${changedFieldId}:`,
        error,
      );
    } finally {
  ...
  function calculateDependencyChain(changedFieldId) {
    ...
    if (calculationInProgress || window.sectionCalculationInProgress) {
      console.warn(
        `[StateManager] Calculation already in progress (orchestration: ${calculationInProgress}, section: ${window.sectionCalculationInProgress}), skipping dependency chain for ${changedFieldId}`,
      );
      return [];
    }
    ...
    try {
      console.log(
        `[StateManager] Starting dependency chain calculation for ${changedFieldId}`,
      );
      ...
      console.log(
        `[StateManager] Calculating ${orderedFields.length} fields in dependency order:`,
        orderedFields,
      );
      ...
      orderedFields.forEach((fieldId) => {
        ...
        if (result !== null) {
          ...
          console.log(
            `[StateManager] ⚠ Skipped ${fieldId} (no calculation registered or calculation failed)`,
          );
        }
      });
      ...
      console.log(
        `[StateManager] Dependency chain calculation complete. Calculated ${calculatedFields.length} fields.`,
      );
    } catch (error) {
      console.error(
        `[StateManager] Error in dependency chain calculation:`,
        error,
      );
    } finally {
      calculationInProgress = false;
    ...
    addListener(fieldId, (newValue, oldValue, changedFieldId, state) => {
      ...
      if (additionalCallback) {
        try {
          ...
          console.error(
            `[StateManager] Error in additional callback for ${fieldId}:`,
            error,
          );
        }
      }
  ...
  function calculateBatchDependencies(changedFieldIds) {
    if (calculationInProgress) {
      console.warn(
        `[StateManager] Calculation already in progress, skipping batch calculation`,
      );
      return [];
    }
    ...
    try {
      console.log(
        `[StateManager] Starting batch dependency calculation for:`,
        changedFieldIds,
      );
      ...
      console.log(
        `[StateManager] Batch calculation complete. Calculated ${allCalculatedFields.length} fields.`,
      );
    } catch (error) {
      console.error(
        `[StateManager] Error in batch dependency calculation:`,
        error,
      );
    } finally {
      calculationInProgress = false;
  ...
  function getCalculationOrder(targetFields = null) {
    ...
    const visit = (fieldId) => {
      if (temp.has(fieldId)) {
        console.warn(
          `[StateManager] Circular dependency detected with field ${fieldId}`,
        );
    ...
    console.log(
      `[StateManager] Calculated topological order for ${order.length} fields:`,
      order,
    );
    return order;
  }
