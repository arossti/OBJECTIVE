...
TEUI.ReferenceToggle = (function () {
  ...
  function initialize() {
    ...
    if (standardSelector) {
      ...
      if (actualSelect) {
        ...
        console.warn(
          `[ReferenceToggle] SELECT element for ${STANDARD_SELECTOR_ID} not found.`,
        );
      }
    } else {
      console.warn(
        `[ReferenceToggle] Standard selector dropdown ${STANDARD_SELECTOR_ID} not found.`,
      );
    }
  }
  ...
  function toggleReferenceView() {
    ...
    if (referenceMode) {
      ...
      if (currentStandard) {
        ...
        if (
          window.TEUI.Calculator &&
          typeof window.TEUI.Calculator.runAllCalculations === "function"
        ) {
  ...
  function triggerFullUIRefreshForModeChange() {
    ...
    if (
      TEUI.FieldManager &&
      typeof TEUI.FieldManager.getAllUserEditableFields === "function"
    ) {
    ...
    try {
      const currentStandardKey =
        TEUI.StateManager.getValue(STANDARD_SELECTOR_ID);

      if (referenceMode) {
        if (!currentStandardKey) {
          console.warn(
            "[ReferenceToggle] No reference standard selected (d_13 is empty). Cannot load reference data for display.",
          );
        } else {
          console.log(
            `[ReferenceToggle] Loading reference data for standard: ${currentStandardKey}`,
          );
      ...
      if (
        !allUserEditableFields ||
        Object.keys(allUserEditableFields).length === 0
      ) {
      ...
      for (const fieldId of fieldIds) {
        ...
        const section01CalculatedFields = [
          "d_6",
          "d_8",
          "e_10",
          "h_6",
        ...
        if (
          TEUI.FieldManager &&
          typeof TEUI.FieldManager.updateFieldDisplay === "function"
        ) {
        ...
        if (element) {
          if (referenceMode) {
            ...
            if (fieldId === "d_13") {
              ...
              if (
                TEUI.AppendixE &&
                typeof TEUI.AppendixE.getFieldBehavior === "function"
              ) {
                const behavior = TEUI.AppendixE.getFieldBehavior(
                  fieldId,
                  currentStandardKey,
                );
                lockField =
                  behavior !== "Independently User-Editable in Reference Mode";
              } else {
                lockField = true; // Lock all other fields by default
              }

              if (
                fieldDef.type === "calculated" ||
                fieldDef.type === "derived"
              ) {
                lockField = true;
              }
            }
      ...
      if (
        TEUI.SectionModules &&
        TEUI.SectionModules.sect05 &&
        typeof TEUI.SectionModules.sect05.updateI39Display === "function"
      ) {
  ...
  function refreshAllSectionsDisplay() {
    if (referenceMode) {
      console.log(
        "[ReferenceToggle] Refreshing all sections display for Reference Mode",
      );
      triggerFullUIRefreshForModeChange();
    }
  }
