...
window.TEUI.SectionModules.sect15 = (function () {
  ...
  function registerITDependsCalculations() {
    if (!window.TEUI?.StateManager?.registerCalculation) {
      console.warn(
        "[S15 IT-DEPENDS] StateManager.registerCalculation not available",
      );
      return;
    }
    ...
    sm.registerCalculation(
      ...
      function () {
        ...
        return (
          gasPrice * d28 + propanePrice * d29 + woodPrice * d31 + oilPrice * d30
        );
      },
      "Other Energy Cost",
    );
    ...
    sm.registerCalculation(
      ...
      function () {
        ...
        let referenceEmissions =
          window.TEUI?.StateManager?.getValue("ref_k_32") ||
          window.TEUI?.StateManager?.getValue("ref_j_32") ||
          getNumericValue("j_32") ||
          0;

        console.log(
          `[S15] d_145 calc - target: ${targetEmissions}, reference: ${referenceEmissions}`,
        );
  ...
  function calculateAll() {
    ...
    if (window.TEUI.sect15.calculationInProgress) {
      console.log(
        "[S15] Calculation already in progress, skipping to prevent recursion",
      );
      return;
    }
  ...
  function calculateValues() {
    try {
      ...
      let ghgReduction_d145 =
        referenceEmissions_j32 > 0
          ? 1 - targetEmissions_k32 / referenceEmissions_j32
          : 0;
  ...
  function initializeEventHandlers() {
    ...
    sm.addListener("d_113", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] d_113 (Heating System) changed, recalculating energy costs",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("d_116", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] d_116 (Cooling Type) changed, recalculating cooling loads",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("i_104", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] i_104 (Total Heat Loss) changed, recalculating TEUI",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("g_101", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] g_101 (U-value Ae) changed, recalculating peak loads",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("g_102", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] g_102 (U-value Ag) changed, recalculating peak loads",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("k_64", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] k_64 (Occupant Cooling) changed, recalculating cooling",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("j_32", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] j_32 (Reference Emissions) changed, updating GHG reduction",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("k_32", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] k_32 (Target Emissions) changed, updating GHG reduction",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("m_121", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] m_121 (Net Ventilation) changed, recalculating TEUI",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("d_122", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] d_122 (Cooling Ventilation) changed, recalculating cooling",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("h_124", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] h_124 (Free Cooling) changed, recalculating cooling",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("d_23", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] d_23 (Coldest Temp) changed, recalculating heating load",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("h_23", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] h_23 (Heating Setpoint) changed, recalculating heating load",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("d_24", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] d_24 (Hottest Temp) changed, recalculating cooling load",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("h_24", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] h_24 (Cooling Setpoint) changed, recalculating cooling load",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("d_14", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] d_14 (Reporting Mode) changed, updating actual values",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("e_10", function (newValue) {
      if (!window.TEUI.sect15.calculationInProgress) {
        console.log(
          "[S15] e_10 (Reference TEUI) changed, updating percentages",
        );
        calculateAll();
      }
    });
  ...
  function testS15_ITDepends() {
    ...
    console.log(
      `${Math.abs(expectedTEUI - actualTEUI) < 0.01 ? "✓" : "❌"} TEUI calculation correct`,
    );
    ...
    setTimeout(() => {
      ...
      console.log(
        `${newD135 !== d136Value ? "✓" : "❌"} Cross-section dependency working`,
      );
