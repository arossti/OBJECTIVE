...
window.TEUI.SectionModules.sect14 = (function () {
  ...
  function setDualEngineValue(fieldId, rawValue, format = "number") {
    ...
    const formattedValue =
      window.TEUI?.formatNumber?.(rawValue, format) ??
      rawValue?.toString() ??
      "N/A";
    const element = document.querySelector(`[data-field-id="${fieldId}"]`);
    if (element) {
  ...
  function registerITDependsCalculations() {
    if (!window.TEUI?.StateManager?.registerCalculation) {
      console.warn(
        "[S14 IT-DEPENDS] StateManager.registerCalculation not available",
      );
      return;
    }
  ...
  function calculateAll() {
    ...
    if (window.TEUI.sect14.calculationInProgress) {
      console.log(
        "[S14] Calculation already in progress, skipping to prevent recursion",
      );
      return;
    }
  ...
  function updateReferenceIndicator() {
    ...
    if (mElement) {
      mElement.textContent =
        window.TEUI?.formatNumber?.(cappedPercentage / 100, "percent") ?? "N/A";
    }
  ...
  function testS14_ITDepends() {
    ...
    console.log(
      `${Math.abs(expectedTEDI - actualTEDI) < 0.01 ? "✓" : "❌"} TEDI calculation correct`,
    );
    ...
    setTimeout(() => {
      ...
      console.log(
        `${newD127 !== d127Value ? "✓" : "❌"} Cross-section dependency working`,
      );
