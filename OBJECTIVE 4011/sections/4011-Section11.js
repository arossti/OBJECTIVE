...
window.TEUI.SectionModules.sect11 = (function () {
  ...
  function flushDOMUpdates() {
    ...
    domUpdateQueue.forEach(
      ({ fieldId, rawValue, formatType, isReferenceMode }, index) => {
    ...
    domUpdateQueue = [];
    console.log(
      `[S11 DOM FLUSH] ✅ ${flushedCount} DOM updates complete, queue cleared`,
    );
  }
  ...
  function setCalculatedValue(fieldId, rawValue, format = "number") {
    ...
    if (!isFinite(rawValue) || rawValue === null || rawValue === undefined) {
      console.log(
        `[S11 CALC SET] ⚠️ Invalid value for ${fieldId}, setting to N/A`,
      );
      const element = document.querySelector(`[data-field-id="${fieldId}"]`);
      if (element) {
  ...
  function calculateComponentRow(
    ...
    try {
      ...
      area = sourceAreaFieldId
        ? getNumericValue(sourceAreaFieldId) || 0
        : getNumericValue(areaFieldId) || 0;
  ...
  function updateReferenceIndicators(rowId) {
    ...
    try {
      ...
      if (baseline.type === "rsi") {
        ...
        const nElementCheck = document.querySelector(
  ...
  function calculateReferenceModel() {
    ...
    componentConfig.forEach((config) => {
      ...
      if (
        result &&
        result.heatloss !== undefined &&
        result.heatgain !== undefined
      ) {
        ...
        const refHeatloss = window.TEUI?.StateManager?.getValue(
          `ref_i_${config.row}`,
        );
        const refHeatgain = window.TEUI?.StateManager?.getValue(
          `ref_k_${config.row}`,
        );
    ...
    if (window.TEUI?.StateManager?.setValue) {
      window.TEUI.StateManager.setValue(
        "ref_e_97",
        validatedPenalty.toString(),
        "calculated",
      );
      window.TEUI.StateManager.setValue(
        "ref_i_97",
        penaltyHeatloss.toString(),
        "calculated",
      );
      window.TEUI.StateManager.setValue(
        "ref_k_97",
        penaltyHeatgain.toString(),
        "calculated",
      );
    }
    ...
    if (window.TEUI?.StateManager?.setValue) {
      ...
      window.TEUI.StateManager.setValue(
        "ref_i_98",
        finalRefLoss.toString(),
        "calculated",
      );
      window.TEUI.StateManager.setValue(
        "ref_k_98",
        finalRefGain.toString(),
        "calculated",
      );
    }
  ...
  function calculateAll() {
    ...
    if (window.sectionCalculationInProgress) {
      console.log(
        "[S11 TRAFFIC COP] ⏸️ Calculation already in progress, skipping...",
      );
      return;
    }
    ...
    try {
      ...
      const currentMode = window.TEUI?.ReferenceToggle?.isReferenceMode?.()
        ? "REFERENCE"
        : "APPLICATION";
  ...
  function onSectionRendered() {
    ...
    if (window.TEUI?.StateManager) {
      editableFields.forEach((fieldId) => {
        const fieldConfig = Object.values(sectionRows)
      ...
      if (window.TEUI?.StateManager?.addListener) {
        window.TEUI.StateManager.addListener(sourceFieldId, () => {
          ...
          if (targetElement) {
            ...
            console.log(
              `[S11 AREA SYNC] 🔄 ${sourceFieldId} changed → updating ${targetFieldId} to ${numericValue}`,
            );
            ...
            targetElement.textContent =
              ...
              numericValue.toString();
            console.log(
              `[S11 AREA SYNC] ✅ DOM updated: ${targetFieldId} = "${targetElement.textContent}"`,
            );
            ...
            if (window.TEUI?.StateManager?.triggerFieldCalculation) {
              ...
              console.log(
                `[S11 AREA SYNC] 🔄 Triggering IT-DEPENDS calculations for row ${row}...`,
              );
              ...
              calculations.forEach((calcId) => {
                try {
                  ...
                  console.warn(
                    `[S11 AREA SYNC] ⚠️ Could not trigger ${calcId}:`,
                    error.message,
                  );
                }
              });
              ...
              try {
                ...
                console.log(
                  `[S11 AREA SYNC] ✅ Triggered totals (d_98, i_98, k_98)`,
                );
              } catch (error) {
                console.warn(
                  `[S11 AREA SYNC] ⚠️ Could not trigger totals:`,
                  error.message,
                );
              }
            } else {
              ...
              console.log(
                `[S11 AREA SYNC] 📊 Fallback to traditional calculateAll()`,
              );
              if (!window.sectionCalculationInProgress) {
                calculateAll();
              }
            ...
            console.warn(
              `[S11 AREA SYNC] ❌ Target element not found: ${targetFieldId}`,
            );
          }
        });

        console.log(
          `[S11 AREA SYNC] 📡 Listener registered: ${sourceFieldId} → d_${targetRow}`,
        );
      }
    });
    ...
    if (window.TEUI?.StateManager?.registerCalculation) {
      ...
      console.warn(
        "[S11 IT-DEPENDS] StateManager.registerCalculation not available",
      );
    }
  ...
  function checkModule() {
    ...
    console.log(
      "Section 11 module loaded:",
      !!window.TEUI?.SectionModules?.sect11,
    );
    console.log(
      "Functions available:",
      Object.keys(window.TEUI?.SectionModules?.sect11 || {}),
    );
  ...
  function diagnoseDOMState() {
    ...
    keyFields.forEach((fieldId) => {
      const element = document.querySelector(`[data-field-id="${fieldId}"]`);
      const stateValue = window.TEUI?.StateManager?.getValue(fieldId);
      const refStateValue = window.TEUI?.StateManager?.getValue(
        `ref_${fieldId}`,
      );
    ...
    console.log(
      "Section Calculation In Progress:",
      !!window.sectionCalculationInProgress,
    );
  ...
  function testDOMUpdate() {
    ...
    testFields.forEach((fieldId) => {
      ...
      if (element && stateValue) {
        ...
        element.textContent = formattedValue;
        console.log(
          `[TEST] ${fieldId}: "${beforeUpdate}" → "${formattedValue}"`,
        );
      } else {
        console.log(
          `[TEST] ${fieldId}: Element=${!!element}, State=${stateValue}`,
        );
      }
    });
  ...
  function testTrafficCop() {
    ...
    if (domUpdateQueue.length > 0) {
      console.log(
        "Queue contents:",
        domUpdateQueue.map((item) => `${item.fieldId}=${item.rawValue}`),
      );
    }
  ...
  function checkStateContamination() {
    ...
    testFields.forEach((fieldId) => {
      ...
      if (appValue && refValue) {
        ...
        console.warn(
          `  ❌ MISSING REF HEMISPHERE: Reference state not created`,
        );
      } else if (!appValue) {
        console.warn(
          `  ❌ MISSING APP HEMISPHERE: Application state not created`,
        );
      }
    });
  ...
  function trackModeSwitch() {
    ...
    toggleButton.onclick = function (event) {
      const beforeMode = window.TEUI?.ReferenceToggle?.isReferenceMode?.()
        ? "REFERENCE"
        : "APPLICATION";
      ...
      setTimeout(() => {
        const afterMode = window.TEUI?.ReferenceToggle?.isReferenceMode?.()
          ? "REFERENCE"
          : "APPLICATION";
    ...
    console.log(
      "Toggle between Reference/Application modes to see contamination tracking",
    );
  }
