...
window.TEUI.SectionModules.sect01 = (function () {
  ...
  function getRefStateValue(fieldId) {
    ...
    if (window.TEUI?.StateManager?.getSessionReferenceValue) {
      const sessionValue =
        window.TEUI.StateManager.getSessionReferenceValue(refFieldId);
  ...
  function getRefNumericValue(fieldId, defaultValue = 0) {
    ...
    if (window.TEUI?.StateManager?.getSessionReferenceValue) {
      ...
      if (sessionRefValue !== null && sessionRefValue !== undefined) {
        const parsed =
          window.TEUI?.parseNumeric?.(sessionRefValue, defaultValue) ??
          defaultValue;
        if (!isNaN(parsed)) {
  ...
  function calculateReferenceModel() {
    ...
    try {
      ...
      if (ref_h136) {
        console.log(
          `[S01] Found Reference TEUI from S15: ${ref_h136} kWh/m²/yr`,
        );
        ...
        const ref_energy =
          getRefStateValue("e_139") || getRefStateValue("j_32") || 0;
        const ref_area = getRefStateValue("h_15") || 1427.2; // standard area

        if (ref_energy > 0 && ref_area > 0) {
          const ref_teui = ref_energy / ref_area;
          console.log(
            `[S01] Calculated Reference TEUI: ${ref_teui} kWh/m²/yr (energy: ${ref_energy}, area: ${ref_area})`,
          );
      ...
      if (window.TEUI?.StateManager?.getSessionReferenceValue) {
        const sessionRefValue =
          window.TEUI.StateManager.getSessionReferenceValue("ref_l_27");
        if (sessionRefValue) {
          ref_l27 = window.TEUI.parseNumeric(sessionRefValue, 0);
          console.log(
            `[S01] 🔐 Found Reference emissions factor from SessionReferenceState: ${ref_l27}`,
          );
        }
      }
      ...
      if (ref_l27 === 0 && window.TEUI?.StateManager?.getReferenceValue) {
        ...
        if (refValue) {
          ...
          console.log(
            `[S01] 🔐 Found Reference emissions factor from ReferenceValue: ${ref_l27}`,
          );
        }
      }
      ...
      if (ref_l27 === 0) {
        ...
        if (stateRefValue) {
          ...
          console.log(
            `[S01] 🔐 Found Reference emissions factor from StateManager ref_l_27: ${ref_l27}`,
          );
        }
      }
      ...
      if (ref_l27 === 0) {
        const appValue =
          window.TEUI?.StateManager?.getApplicationValue?.("l_27") ||
          window.TEUI?.StateManager?.getValue?.("l_27");
        if (appValue) {
          ref_l27 = window.TEUI.parseNumeric(appValue, 0);
          console.log(
            `[S01] ⚠️ Falling back to Application emissions factor: ${ref_l27}`,
          );
        } else {
      ...
      if (ref_energy > 0 && ref_area > 0) {
        ...
        const ref_carbon_intensity =
          Math.round((ref_emissions / ref_area) * 10) / 10;
        console.log(
          `[S01] Calculated Reference Carbon Intensity: ${ref_carbon_intensity} kgCO2e/m²/yr (energy: ${ref_energy}, emissions factor: ${ref_l27}, total emissions: ${ref_emissions}, area: ${ref_area})`,
        );
        ...
        console.log(
          "[S01] Using standard Reference Carbon Intensity: 10.5 kgCO2e/m²/yr",
        );
      }
      ...
      if (ref_service_life > 0) {
        const ref_lifetime =
          Math.round((ref_embodied / ref_service_life + ref_annual) * 10) / 10;
        console.log(
          `[S01] Calculated Reference Lifetime Carbon: ${ref_lifetime} kgCO2e/m²/yr (embodied: ${ref_embodied}, service life: ${ref_service_life}, annual: ${ref_annual})`,
        );
        ...
        console.log(
          "[S01] Using standard Reference Lifetime Carbon: 17.1 kgCO2e/m²/yr",
        );
      }
    } finally {
  ...
  function calculateTargetModel() {
    ...
    try {
      ...
      if (h136 && parseFloat(h136) > 0) {
        ...
        if (target_area > 0) {
          const target_teui =
            Math.round((target_energy / target_area) * 10) / 10;
          console.log(
            `[S01] Calculated Target TEUI: ${target_teui} kWh/m²/yr (energy: ${target_energy}, area: ${target_area})`,
          );
      ...
      if (target_area > 0) {
        const target_carbon_intensity =
          Math.round((target_emissions / target_area) * 10) / 10;
        console.log(
          `[S01] Calculated Target Carbon Intensity: ${target_carbon_intensity} kgCO2e/m²/yr (emissions: ${target_emissions}, area: ${target_area}, emissions factor: ${app_l27})`,
        );
        setTargetValue("h_8", target_carbon_intensity);
      } else {
        console.log(
          "[S01] Cannot calculate Target Carbon Intensity: area is zero",
        );
      ...
      if (target_service_life > 0) {
        const target_lifetime =
          Math.round(
            (target_embodied / target_service_life + target_annual) * 10,
          ) / 10;
        console.log(
          `[S01] Calculated Target Lifetime Carbon: ${target_lifetime} kgCO2e/m²/yr (embodied: ${target_embodied}, service life: ${target_service_life}, annual: ${target_annual})`,
        );
        setTargetValue("h_6", target_lifetime);
      } else {
        console.log(
          "[S01] Cannot calculate Target Lifetime Carbon: service life is zero",
        );
      ...
      if (actual_emissions > 0 && target_area > 0) {
        const actual_carbon_intensity =
          Math.round((actual_emissions / target_area) * 10) / 10;
        console.log(
          `[S01] Calculated Actual Carbon Intensity: ${actual_carbon_intensity} kgCO2e/m²/yr`,
        );
      ...
      if (typeof actual_annual === "number" && target_service_life > 0) {
        const actual_lifetime =
          Math.round(
            (target_embodied / target_service_life + actual_annual) * 10,
          ) / 10;
        console.log(
          `[S01] Calculated Actual Lifetime Carbon: ${actual_lifetime} kgCO2e/m²/yr`,
        );
        setCalculatedValue("k_6", actual_lifetime);
      } else {
        console.log(
          "[S01] No Actual Emissions data available for lifetime calculation",
        );
  ...
  function updateTEUIDisplay() {
    ...
    const d6RefFormatted =
      window.TEUI?.formatNumber?.(
        window.TEUI?.parseNumeric?.(d6RefRaw, 24.4),
        "number-1dp",
      ) ?? d6RefRaw;
    const d8RefFormatted =
      window.TEUI?.formatNumber?.(
        window.TEUI?.parseNumeric?.(d8RefRaw, 17.4),
        "number-1dp",
      ) ?? d8RefRaw;
    ...
    console.log(
      `[S01 DISPLAY] 🔵 Reference Column Values: d_6=${d6RefFormatted}, d_8=${d8RefFormatted}, e_10=${e10RefFormatted}`,
    );
    ...
    console.log(
      `[S01 DISPLAY] 🟢 Target Column Values: h_6=${h6Formatted}, h_8=${h8Formatted}, h_10=${h10Formatted}`,
    );
    ...
    if (isUtilityMode) {
      ...
      console.log(
        `[S01 DISPLAY] 📋 Actual Column Values: k_6=${k6Formatted}, k_8=${k8Formatted}, k_10=${k10Raw}`,
      );
  ...
  function calculateAll() {
    ...
    if (window.TEUI.sect01.calculationInProgress) {
      console.log(
        "[S01] Calculation already in progress, skipping to prevent recursion",
      );
      return;
    }
  ...
  function initializeEventHandlers() {
    ...
    sm.addListener("ref_h_136", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] ref_h_136 (S15 Reference TEUI) changed, updating Reference values",
        );
    ...
    sm.addListener("h_136", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] h_136 (S15 Target TEUI) changed, checking for updates",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("j_32", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] j_32 (Target Energy) changed, updating calculations",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("k_32", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] k_32 (Target Emissions) changed, updating calculations",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("f_32", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] f_32 (Actual Energy) changed, updating actual values",
        );
    ...
    sm.addListener("g_32", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] g_32 (Actual Emissions) changed, updating actual values",
        );
        calculateTargetModel();
      }
    });
    ...
    sm.addListener("i_41", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] i_41 (Embodied Carbon) changed, updating lifetime carbon",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("h_13", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] h_13 (Service Life) changed, updating lifetime carbon",
        );
        calculateAll();
      }
    });
    ...
    sm.addListener("ref_j_32", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] ref_j_32 (Reference Emissions) changed, updating Reference values",
        );
        calculateReferenceModel();
      }
    });
    ...
    sm.addListener("ref_e_139", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] ref_e_139 (Reference Energy) changed, updating Reference TEUI",
        );
        calculateReferenceModel();
      }
    });
    ...
    sm.addListener("ref_h_15", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] ref_h_15 (Reference Area) changed, updating Reference calculations",
        );
        calculateReferenceModel();
      }
    });
    ...
    sm.addListener("ref_i_41", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] ref_i_41 (Reference Embodied) changed, updating Reference lifetime",
        );
        calculateReferenceModel();
      }
    });
    ...
    sm.addListener("ref_h_13", function (newValue) {
      if (!window.TEUI.sect01.calculationInProgress) {
        console.log(
          "[S01] ref_h_13 (Reference Service Life) changed, updating Reference lifetime",
        );
        calculateReferenceModel();
      }
    });
  ...
  function registerDependencies() {
    if (!window.TEUI.StateManager) {
      console.warn(
        "[S01] StateManager not available for dependency registration",
      );
      return;
    }
    const sm = window.TEUI.StateManager;
  ...
  function registerITDependsCalculations() {
    if (!window.TEUI?.StateManager?.registerCalculation) {
      console.warn(
        "[S01 IT-DEPENDS] StateManager.registerCalculation not available",
      );
      return;
    }
    ...
    sm.registerCalculation(
      ...
      function () {
        ...
        const refEmissions =
          sm.getValue("ref_j_32") || sm.getValue("j_32") || 0;
    ...
    sm.registerCalculation(
      ...
      function () {
        ...
        return (
          Math.round((embodiedCarbon / serviceLife + annualCarbon) * 10) / 10
        );
      },
  ...
  function testS01_ITDepends() {
    ...
    expectedCalculations.forEach((calcId) => {
      ...
      console.log(
        `${isRegistered ? "✓" : "❌"} ${calcId} registered: ${isRegistered}`,
      );
    ...
    console.log(
      `${refTEUI !== targetTEUI ? "✓" : "❌"} Reference and Target values are different`,
    );
    ...
    setTimeout(() => {
      ...
      console.log(
        `${Math.abs(newH10 - expectedTEUI) < 0.1 ? "✓" : "❌"} Cross-section dependency working`,
      );
    ...
    sm.addListener("ref_k_32", function (newValue) {
      ...
      if (window.TEUI.ReferenceToggle?.isReferenceMode?.()) {
        ...
        if (refEmissions > 0 && area > 0) {
          const refCarbonIntensity =
            Math.round((refEmissions / area) * 10) / 10;
