...
window.TEUI.SectionModules.sect04 = (function () {
  ...
  function getRefNumericValue(fieldId, defaultValue = 0) {
    ...
    if (window.TEUI?.StateManager?.getSessionReferenceValue) {
      const sessionValue = window.TEUI.StateManager.getSessionReferenceValue(
        `ref_${fieldId}`,
      );
      if (sessionValue !== null && sessionValue !== undefined) {
        return (
          window.TEUI?.parseNumeric?.(sessionValue, defaultValue) ??
          defaultValue
        );
      }
    }
    ...
    if (window.TEUI?.StateManager?.getValue) {
      ...
      if (prefixedValue !== null && prefixedValue !== undefined) {
        return (
          window.TEUI?.parseNumeric?.(prefixedValue, defaultValue) ??
          defaultValue
        );
      }
    }
    ...
    if (window.TEUI?.StateManager?.activeReferenceDataSet) {
      const activeDataValue =
        window.TEUI.StateManager.activeReferenceDataSet[fieldId];
      if (activeDataValue !== null && activeDataValue !== undefined) {
        return (
          window.TEUI?.parseNumeric?.(activeDataValue, defaultValue) ??
          defaultValue
        );
      }
    }
        ...
        if (cell.fieldId && cell.type) {
          ...
          [
            ...
            if (cell[prop] !== undefined)
              fields[cell.fieldId][prop] = cell[prop];
          });
        }
      });
    });
  ...
  function createLayoutRow(row) {
    ...
    columns.forEach((col) => {
      if (row.cells && row.cells[col]) {
        ...
        ["getOptions", "section", "dependencies"].forEach(
          (prop) => delete cell[prop],
        );
  ...
  function setupCrossSectionDependencies() {
    ...
    if (sm.addListener) {
      ...
      sm.addListener("e_51", function () {
        calculateAll();
      }); // DHW Gas
      sm.addListener("k_54", function () {
        calculateAll();
      }); // DHW Oil
      sm.addListener("h_115", function () {
        calculateAll();
      }); // Heating Gas
      sm.addListener("f_115", function () {
        calculateAll();
      }); // Heating Oil
    }
    ...
    document.removeEventListener(
      "teui-reference-mode-changed",
      refreshUIOnModeChange,
    );
  ...
  function updateGridIntensityForMode(mode) {
    ...
    const provinceStateValue =
      window.TEUI.StateManager?.getApplicationValue("d_19");
  ...
  function registerITDependsCalculations() {
    ...
    sm.registerCalculation(
      ...
      function () {
        ...
        // Get reporting year
        let reportingYear = 2022;
        const yearStateValue =
          window.TEUI.StateManager?.getApplicationValue("h_12");
        if (yearStateValue) {
          const parsedYear = parseInt(yearStateValue);
          if (!isNaN(parsedYear) && parsedYear >= 2015 && parsedYear <= 2041) {
            reportingYear = parsedYear;
  ...
  function testITDependsCalculations() {
    ...
    setTimeout(() => {
      ...
      console.log(
        `[S04 Test] Expected f_27: ${expected_f27.toFixed(2)} kWh/yr`,
      );

      const g27 = window.TEUI.parseNumeric(
        window.TEUI.StateManager.getValue("g_27"),
        0,
      );
      console.log(
        `[S04 Test] g_27 (Actual Electricity Emissions): ${g27.toFixed(2)} kgCO2e/yr`,
      );
      const expected_g27 = (expected_f27 * 51) / 1000; // Net electricity * factor / 1000
      console.log(
        `[S04 Test] Expected g_27: ${expected_g27.toFixed(2)} kgCO2e/yr`,
      );

      // Verify subtotals
      const f32 = window.TEUI.parseNumeric(
        window.TEUI.StateManager.getValue("f_32"),
        0,
      );
      console.log(
        `[S04 Test] f_32 (Total Actual Energy): ${f32.toFixed(2)} kWh/yr`,
      );

      const g32 = window.TEUI.parseNumeric(
        window.TEUI.StateManager.getValue("g_32"),
        0,
      );
      console.log(
        `[S04 Test] g_32 (Total Actual Emissions): ${g32.toFixed(2)} kgCO2e/yr`,
      );

      const j32 = window.TEUI.parseNumeric(
        window.TEUI.StateManager.getValue("j_32"),
        0,
      );
      console.log(
        `[S04 Test] j_32 (Total Target Energy): ${j32.toFixed(2)} kWh/yr`,
      );

      const k32 = window.TEUI.parseNumeric(
        window.TEUI.StateManager.getValue("k_32"),
        0,
      );
      console.log(
        `[S04 Test] k_32 (Total Target Emissions): ${k32.toFixed(2)} kgCO2e/yr`,
      );

      // Verify per capita values
      const d34 = window.TEUI.parseNumeric(
        window.TEUI.StateManager.getValue("d_34"),
        0,
      );
      console.log(
        `[S04 Test] d_34 (Actual Per Capita Energy): ${d34.toFixed(2)} kWh/person`,
      );
      const expected_d34 = f32 / 4; // Total energy / occupants
      console.log(
        `[S04 Test] Expected d_34: ${expected_d34.toFixed(2)} kWh/person`,
      );
