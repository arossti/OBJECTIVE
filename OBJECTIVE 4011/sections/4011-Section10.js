...
window.TEUI.SectionModules.sect10 = (function () {
  ...
  function calculateReferenceModel() {
    ...
    try {
      ...
      orientationConfig.forEach((rowId) => {
        ...
        setDualEngineValue(
          `i_${rowId}`,
          results.heatingGains,
          "number-2dp-comma",
        );
        setDualEngineValue(
          `k_${rowId}`,
          results.coolingGains,
          "number-2dp-comma",
        );
      });
      ...
      setDualEngineValue(
        "i_79",
        subtotalResults.heatingGains,
        "number-2dp-comma",
      );
      setDualEngineValue(
        "k_79",
        subtotalResults.coolingGains,
        "number-2dp-comma",
      );
      ...
      console.error(
        "[Section10] Error in Reference Model calculations:",
        error,
      );
    }
  }
  ...
  function calculateApplicationModel() {
    ...
    try {
      ...
      console.error(
        "[Section10] Error in Application Model calculations:",
        error,
      );
    }
  ...
  function calculateOrientationGains(rowId, isReferenceCalculation = false) {
    try {
      ...
      if (isReferenceCalculation) {
        ...
        winterShadingDecimal =
          window.TEUI.parseNumeric(getRefFieldValue(`g_${rowId}`), 0) / 100;
        summerShadingDecimal =
          window.TEUI.parseNumeric(getRefFieldValue(`h_${rowId}`), 100) / 100;
      } else {
        ...
        winterShadingDecimal =
          window.TEUI.parseNumeric(getAppFieldValue(`g_${rowId}`), 0) / 100;
        summerShadingDecimal =
          window.TEUI.parseNumeric(getAppFieldValue(`h_${rowId}`), 100) / 100;
      }
      ...
      console.error(
        `Error calculating orientation gains for row ${rowId}:`,
        error,
      );
  ...
  function onSectionRendered() {
    ...
    if (window.TEUI?.StateManager?.registerCalculation) {
      ...
      try {
            ...
            if (utilizationMethod === "NRC 0%") {
              ...
              if (totalLosses > 0) {
                ...
                if (Math.abs(gamma - 1) < 1e-9) {
                  ...
                  const gamma_a_plus_1 = Math.pow(gamma, a + 1);
                  utilizationFactor = (1 - gamma_a) / (1 - gamma_a_plus_1);
                  utilizationFactor = Math.max(
                    0,
                    Math.min(1, utilizationFactor),
                  );
                }
              } else {
        ...
        console.log(
          "[S10 IT-DEPENDS] ✅ Successfully registered calculation functions",
        );
        console.log(
          "[S10 IT-DEPENDS] Registered: i_79, k_79, e_80, g_80, i_80, i_82 + orientation gains",
        );
      ...
      console.warn(
        "[S10 IT-DEPENDS] StateManager.registerCalculation not available",
      );
    }
  ...
  function testS10_ITDepends() {
    ...
    console.log(
      `Registration Summary: ${registrationsPassed}/${allCalculations.length} calculations registered`,
    );
    ...
    executionTests.forEach((test) => {
      ...
      const changed =
        Math.abs(afterValue - beforeValue) > 0.001 ||
        (beforeValue === 0 && afterValue >= 0);

      console.log(
        `${success && changed ? "✓" : "❌"} ${test.id} (${test.desc}): ${beforeValue.toFixed(2)} → ${afterValue.toFixed(2)}`,
      );
    ...
    sampleOrientationTests.forEach((test) => {
      ...
      console.log(
        `${success && meaningful ? "✓" : "❌"} ${test.id} (${test.desc}): ${beforeValue.toFixed(2)} → ${afterValue.toFixed(2)}`,
      );
    ...
    console.log(
      `${utilizationWorking ? "✓" : "❌"} Utilization factor valid: ${afterUtilization.toFixed(3)} (0.0-1.0 range)`,
    );
    ...
    console.log("\n=== TEST RESULTS ===");
    console.log(
      `✅ Registration: ${registrationsPassed}/${allCalculations.length} passed`,
    );
    console.log(
      `${executionsPassed >= 4 ? "✅" : "❌"} Key Calculations: ${executionsPassed}/${executionTests.length} passed`,
    );
    console.log(
      `${orientationPassed >= 1 ? "✅" : "❌"} Orientation Tests: ${orientationPassed}/${sampleOrientationTests.length} passed`,
    );
    console.log(
      `${utilizationWorking ? "✅" : "❌"} Utilization Logic: ${utilizationWorking ? "PASSED" : "FAILED"}`,
    );

    const overallPassed =
      registrationsPassed === allCalculations.length &&
      executionsPassed >= 4 &&
      orientationPassed >= 1 &&
      utilizationWorking;
    ...
    console.log(
      "🚀 Section 10 IT-DEPENDS ready for dependency-driven calculations!",
    );
  ...
  function checkS10_ITDependsStatus() {
    ...
    const keyRegistered = keyCalculations.filter(
      (id) => sm.hasCalculation && sm.hasCalculation(id),
    ).length;
    const orientationRegistered = orientationCalculations.filter(
      (id) => sm.hasCalculation && sm.hasCalculation(id),
    ).length;

    console.log(
      `📊 Key Calculations: ${keyRegistered}/${keyCalculations.length} registered`,
    );
    console.log(
      `📊 Orientation Calculations: ${orientationRegistered}/${orientationCalculations.length} registered`,
    );
    console.log(
      `🔄 Migration Status: HYBRID MODE (calculations registered, listeners active)`,
    );
    ...
    const totalExpected =
      keyCalculations.length + orientationCalculations.length;
    if (totalRegistered === totalExpected) {
      console.log(
        "\n🚀 Performance Status: Ready for dependency-driven calculations",
      );
      console.log(
        "💡 Next Step: Test individual calculations with triggerFieldCalculation()",
      );
    } else {
      console.log(
