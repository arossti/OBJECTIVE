...
window.TEUI.SectionModules.sect06 = (function () {
   ...
   */
  function setCalculatedValueHelper(
    fieldId,
    rawValue,
    ...
    if (isNaN(numericValue)) {
      ...
      const element = document.querySelector(`[data-field-id="${fieldId}"]`);
      if (element)
        element.textContent = window.TEUI?.formatNumber?.(0, formatType) ?? "0";
      ...
      if (!field.hasEditableListeners) {
        ...
        field.addEventListener("focus", function () {
          ...
          this.dataset.originalValue =
            window.TEUI.StateManager.getValue(this.dataset.fieldId) || "0";
        });
  ...
  function handleEditableBlur(element) {
    ...
    if (!isNaN(numericValue)) {
      ...
      console.warn(
        `Invalid input for ${fieldId}: "${newValueText}". Reverted to ${element.textContent}.`,
      );
    }
  }
