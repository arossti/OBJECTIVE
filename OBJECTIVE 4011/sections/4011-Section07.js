...
window.TEUI.SectionModules.sect07 = (function () {
  ...
  function setCalculatedValue(
    ...
    if (rawValue === "N/A") {
      ...
      formattedValue =
        window.TEUI?.formatNumber?.(rawValue, formatType) ?? valueToStore; // Fallback to stored value
    }
  ...
  function handleEditableBlur(event) {
    ...
    if (isNaN(numericValue)) {
      // Handle invalid numeric input -> default to 0 or revert
      console.warn(
        `Invalid input for ${fieldId}: "${rawTextValue}". Reverting or defaulting.`,
      );
  ...
  function calculateWaterUseForMode(mode = "current") {
    ...
    setDualEngineValue("h_49", litersPerPersonDay, litersPerPersonDay, mode);
    setDualEngineValue("i_49", annualWaterUse, annualWaterUse, mode);
    setDualEngineValue(
      "h_50",
  ...
  function calculateWaterUse() {
    ...
    const annualWaterUse = litersPerPersonDay * occupants * 365;
    setDualEngineValue("i_49", annualWaterUse, annualWaterUse, "current");
    const hotWaterLitersPerDay = litersPerPersonDay * 0.4;
    setDualEngineValue(
      "h_50",
      hotWaterLitersPerDay,
  ...
  function calculateHeatingSystem(hotWaterEnergyDemand_j50) {
    ...
    let efficiency = !isNaN(efficiencyInput_d52)
      ? efficiencyInput_d52 / 100
      : 1.0;
    setDualEngineValue("e_52", efficiency, efficiency, "current");
    ...
    let recoveryPercent = !isNaN(recoveryOption_d53)
      ? recoveryOption_d53 / 100
      : 0;
    ...
    switch (systemType) {
      ...
      case "Gas":
        ...
        const adjustedDemand =
          netDemandAfterRecovery_j52 * (1 - recoveryPercent);
    ...
    let netElectricalDemand_k51 =
      systemType !== "Gas" && systemType !== "Oil" ? netDemand : 0;
    setDualEngineValue(
      "k_51",
      netElectricalDemand_k51,
      netElectricalDemand_k51,
      "current",
    );
    ...
    if (efficiency <= 1) {
      systemLosses_d54 =
        hotWaterEnergyDemand_j50 *
        (waterUseMethod_d49 === "PHPP Method" ? 0.25 : 0.1);
    }
    setDualEngineValue("d_54", systemLosses_d54, systemLosses_d54, "current");
    ...
    const efficiencyPercentRaw =
      standardCOP !== 0 ? efficiency / standardCOP : 0;
    setDualEngineValue(
      "n_52",
      efficiencyPercentRaw,
      efficiencyPercentRaw,
      "current",
    );
    setDualEngineValue("n_53", recoveryPercent, recoveryPercent, "current");
  ...
  function calculateJ54(mode = "current") {
    ...
    const systemType = isReferenceMode
   ...
   * MODE-AWARE: Uses appropriate state values based on calculation mode
   */
  function calculateDHWEmissions(mode = "current") {
    const isReferenceMode = mode === "reference";
    ...
    setDualEngineValue("k_49", emissions_kg, emissions_kg, mode);
  ...
  function calculateAll() {
    ...
    if (window.TEUI && window.TEUI.StateManager) {
      window.TEUI.StateManager.setValue(
        "h_69",
        heatingResults.systemLosses.toString(),
        "calculated",
      );
    }
    ...
    try {
      ...
      const refHeatingResults = calculateHeatingSystem(
        refWaterResults.hotWaterEnergyDemand,
      );
  ...
  function onSectionRendered() {
    ...
    const initialWaterMethod =
      (isReferenceMode ? getRefFieldValue("d_49") : getFieldValue("d_49")) ||
      "User Defined";
    const initialSystemType =
      (isReferenceMode ? getRefFieldValue("d_51") : getFieldValue("d_51")) ||
      "Heatpump";
  ...
  function updateSection7Visibility(waterMethod, systemType) {
    ...
    const g54Cell = document.querySelector(
      '.data-table tr[data-id="W.6.1"] td:nth-child(7)',
    );
    if (g54Cell) g54Cell.classList.toggle("disabled-input", !isOil);
    const l54LabelCell = document.querySelector(
      '.data-table tr[data-id="W.6.1"] td:nth-child(12)',
    );
    if (l54LabelCell) l54LabelCell.classList.toggle("disabled-input", !isOil);

    setFieldGhosted("k_54", !isOil);
    const h54Cell = document.querySelector(
      '.data-table tr[data-id="W.6.1"] td:nth-child(8)',
    );
    ...
    const f52Cell_label = document.querySelector(
      '.data-table tr[data-id="W.4.1"] td:nth-child(6)',
    );
    if (f52Cell_label)
      f52Cell_label.classList.toggle("disabled-input", isFossilFuel);

    setFieldGhosted("k_52", !isFossilFuel);
    const l52Cell_label = document.querySelector(
      '.data-table tr[data-id="W.4.1"] td:nth-child(12)',
    );
    if (l52Cell_label)
      l52Cell_label.classList.toggle("disabled-input", !isFossilFuel);
  }
  ...
  function handleSliderChange(e) {
    ...
    if (
      ...
      window.TEUI.StateManager.setValue(fieldId, value, "user-modified");
      if (e.type === "change") {
        // Only calculate fully on release
        calculateAll();
      }
    }
    if (fieldId === "d_52") {
      // Special handling for d_52 visibility
  ...
  function handleReferenceStandardChange(newStandardKey) {
    ...
    const standardData =
      window.TEUI.ReferenceValues.getStandardData?.(newStandardKey);
    if (!standardData) {
      ...
      if (standardData[fieldId] !== undefined) {
        ...
        if (window.TEUI?.StateManager?.setReferenceValue) {
          window.TEUI.StateManager.setReferenceValue(
            fieldId,
            standardData[fieldId],
            "standard-update",
          );
        }
        console.log(
          `[Section07] Updated reference ${fieldId} to ${standardData[fieldId]} for ${newStandardKey}`,
        );
      }
    });
